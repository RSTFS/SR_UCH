...
//本文件反射的游戏私有成员（游戏更新改名时：日志告警 + 可在 cfg [Reflection] 段填新名恢复）
        SR.RefOverride("Level.GetSpawnPosition", "自定义重生点（取原生出生点）");
        SR.RefOverride("FreePlayControl.resetPlayerCharacter", "重生延迟抑制（补丁目标）");
    ...
    public void Initialize(MainPlugin plugin) {
        ...
        try { Harmony.CreateAndPatchAll(typeof(Respawn), (string)null); }
        catch (Exception e) { MainPlugin.ModLogger.LogError("重生（无敌/延迟/自定义点） 补丁注册失败: " + e.Message); }
    }
        ...
        public static void ReadDefaultSpawn() {
            ...
            try {
                if (_getSpawnPos == null)
                    _getSpawnPos = SR.RefMethod(typeof(Level), "GetSpawnPosition", new[] { typeof(float) }, "自定义重生点（取原生出生点）");
        ...
        static void OnDeath(Character __instance) {
            ...
            if (!_resetResolved) {
                _resetResolved = true;
                _reset = SR.RefMethod(typeof(FreePlayControl), "resetPlayerCharacter", new[] { typeof(Character), typeof(bool) }, "重生延迟抑制（补丁目标）");
                if (_reset == null)
        ...
        //默认值必须是 int.MaxValue（=不抑制，走游戏原版自动重生），不是 -1（=抑制）。
        //_suppress 由上面的 UpdateSuppress Prefix 每帧赋值；万一该 Prefix 没跑（补丁目标改名/异常、
        //游戏更新后失配），字段会停在初始值 —— 初始值若为 -1，游戏自动重生就被永久抑制、
        //玩家死了不再生（比"延迟失效"严重得多）。用"失效即恢复原版行为"的默认值更安全。
        private static int _suppress = int.MaxValue;
        ...
        private class DelayComponent : MonoBehaviour {
            ...
            private IEnumerator Accelerate(Character c) {
                ...
                //注：当前 OnDeath 在挑战模式已提前 return、不会 Schedule，故此分支实际不可达；
                //保留作安全网 —— 若将来 OnDeath 放宽条件放行挑战模式，这里仍能拦住自动重试。
