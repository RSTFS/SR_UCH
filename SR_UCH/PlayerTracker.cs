...
namespace SR_UCH.Tweaks {
    ...
    public class PlayerTracker : ITweak {
        ...
        private static void SelfReg() {
            ...
            SR.LocDesc("Player Tracker", "Tracking Length", "轨迹长度：记录多长时间的移动轨迹（60 格 ≈ 1 秒）", "How long of a movement trail to record (60 ticks ≈ 1 second)");
            SR.LocKey("Player Tracker", "Skip Frames", "跳帧数", null);
            SR.LocDesc("Player Tracker", "Skip Frames", "跳帧数：每跳过 N 帧才记录一个位置点（越大轨迹越稀疏）", "Record a point every N frames (higher = sparser trail)");
            SR.LocKey("Player Tracker", "Line Start Width", "起点宽度", null);
            SR.LocDesc("Player Tracker", "Line Start Width", "轨迹起点宽度（越靠近当前位置越宽/越细）", "Trail start width (near the current position)");
            SR.LocKey("Player Tracker", "Line End Width", "终点宽度", null);
            SR.LocDesc("Player Tracker", "Line End Width", "轨迹终点宽度（最旧位置点的宽度）", "Trail end width (at the oldest point)");
            SR.LocKey("Player Tracker", "Teleport Threshold", "瞬移清除距离", null);
            SR.LocDesc("Player Tracker", "Teleport Threshold", "瞬移清除距离（格数）：角色一帧内移动超过该距离（被瞬移/传送/派对盒选道具）时，删除已绘制的轨迹重新开始，避免拖出贯穿全图的长线。最小 1。", "Teleport clear distance (units): if a character moves farther than this in one frame (teleport/warp/party-box pick), delete the drawn trail and restart, so no giant line spans the map. Min 1.");
            SR.LocKey("Player Tracker", "Enabled", "移动轨迹总开关", null);
            SR.LocDesc("Player Tracker", "Enabled", "移动轨迹总开关：开启后在画面上绘制其他玩家的移动轨迹线", "Master switch: draw movement trails for other players on screen");
        }
