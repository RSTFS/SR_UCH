...
// ==== 分区：Map（俯视地图视图：相机取景 / 拖拽缩放 / 传送 / 重生点标记 / 地图窗口）====
        ...
        private static void DrawMapWindow() {
            ...
            if (Input.GetKeyDown(KeyCode.T)) {
                ...
                if (SpawnPoints.RemoveNearest(w2, 1.5f)) {
                    _activePoint = -1;
                    _mapToast = T("已删除该处重生点（剩 " + SpawnPoints.CustomPoints.Count + "）", "Removed spawn point (" + SpawnPoints.CustomPoints.Count + " left)");
                } else {
                    _mapToast = T("此处没有自定义重生点（游戏自带起点不可删）", "No custom spawn point here (the default start can't be removed)");
                }
                _mapToastUntil = Time.unscaledTime + 1.5f;
            }
            //操作提示（暂停时也可见）
