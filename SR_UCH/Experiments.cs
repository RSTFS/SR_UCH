...
//本文件反射的游戏私有成员（游戏更新改名时：日志告警 + 可在 cfg [Reflection] 段填新名恢复）
			SR.RefOverride("LevelSelectController.unlockQuestionMarks", "树屋问号（添加/删除/全部添加/清除全部）");
			SR.RefOverride("LevelSelectController.CharacterUnlocks", "树屋问号（角色解锁判定）");
			SR.RefOverride("LevelSelectController.LevelUnlocks", "树屋问号（关卡解锁判定）");
			SR.RefOverride("LevelSelectController.OutfitUnlocks", "树屋问号（外观解锁判定）");
			SR.RefOverride("LevelSelectController.SetUnlockForPlayer", "树屋问号（写入解锁状态）");
			SR.RefOverride("LevelSelectController.SendUnlockMessageFromClient", "树屋问号（客户端发解锁消息）");
		...
		public static void Render() {
			...
			GUILayout.Space(SR.Ctl.Sc(2));
			GUILayout.BeginHorizontal();
			if (SR.Ctl.HotkeyButton("exp.add_question", SR.T("添加问号", "Add ?"), SR.T("给上方选中的关卡添加问号（进入该地图有解锁盒子）", "Mark the selected level with a question mark"), Experiments.ApplyQuestionMark, SR.Ctl.Sc(140), SR.Ctl.Sc(30))) {
				Experiments.ApplyQuestionMark();
			}
			if (SR.Ctl.HotkeyButton("exp.remove_question", SR.T("删除问号", "Remove ?"), SR.T("删除上方选中的关卡的门上的问号", "Remove the question mark on the selected level"), Experiments.RemoveQuestionMark, SR.Ctl.Sc(140), SR.Ctl.Sc(30))) {
			...
			GUILayout.Space(SR.Ctl.Sc(2));
			GUILayout.BeginHorizontal();
			if (SR.Ctl.HotkeyButton("exp.add_all_questions", SR.T("全部添加问号", "Add all ?"), SR.T("为当前所有已解锁的关卡添加问号（未解锁的自动跳过）", "Add a question mark to every unlocked level"), Experiments.AddAllQuestionMarks, SR.Ctl.Sc(140), SR.Ctl.Sc(30))) {
				Experiments.AddAllQuestionMarks();
			}
			if (SR.Ctl.HotkeyButton("exp.clear_questions", SR.T("一键清除全部问号", "Clear all ?"), SR.T("清空树屋里所有关卡门上的问号", "Clear every question mark in the treehouse"), Experiments.ClearQuestionMarks, SR.Ctl.Sc(140), SR.Ctl.Sc(30))) {
		...
		public void Initialize(MainPlugin plugin)
		{
			...
			catch (Exception e) { MainPlugin.ModLogger.LogError("实验（树屋问号/GC） 补丁注册失败: " + e.Message); }
		}
		...
		public static void ApplyQuestionMark()
		{
			try
			{
				...
				if (flag2)
				{
					MethodInfo methodInfo = SR.RefMethod(typeof(LevelSelectController), "SetUnlockForPlayer", "树屋问号（写入解锁状态）");
					if (methodInfo != null)
					{
				...
				{
					MethodInfo methodInfo2 = SR.RefMethod(typeof(LevelSelectController), "SendUnlockMessageFromClient", "树屋问号（客户端发解锁消息）");
					if (methodInfo2 != null)
					{
		...
		private static bool TryPickUnlock(LevelSelectController lsc, out UnLockInfo result)
		{
			...
			try
			{
				...
				try
				{
					FieldInfo fieldInfo = SR.RefField(typeof(LevelSelectController), "CharacterUnlocks", "树屋问号（角色解锁判定）");
					if (fieldInfo != null)
					{
				...
				try
				{
					FieldInfo fieldInfo2 = SR.RefField(typeof(LevelSelectController), "LevelUnlocks", "树屋问号（关卡解锁判定）");
					if (fieldInfo2 != null)
					{
				...
				try
				{
					FieldInfo fieldInfo3 = SR.RefField(typeof(LevelSelectController), "OutfitUnlocks", "树屋问号（外观解锁判定）");
					if (fieldInfo3 != null)
					{
		...
		public static void RemoveQuestionMark()
		{
			try
			{
				...
				FieldInfo fieldInfo = SR.RefField(typeof(LevelSelectController), "unlockQuestionMarks", "树屋问号（添加/删除/全部添加/清除全部）");
				if (fieldInfo == null)
				{
					return;
				}
