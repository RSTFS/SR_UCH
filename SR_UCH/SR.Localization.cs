...
// 新增翻译：在此文件的字典中按 "分区\t键名" 添加 中/英 条目即可；新功能文本用 Loc.T(zh, en)。
        ...
        private static readonly Dictionary<string, string> _keyZh = new Dictionary<string, string> {
            ...
            { "Builder Enhancements\tGrid Override", "自由放置" },
            { "Builder Enhancements\tGrid Toggle Key", "自由放置开关" },
            ...
            { "Player Tracker\tTeleport Threshold", "瞬移清除距离" },
            ...
            { "实验\tQuick Retry", "死后自动重试" },
            { "实验\tQuick Retry Fast", "快速重试" },
            { "实验\tQuick Retry Hold", "等待秒数" },
        ...
        private static readonly Dictionary<string, string> _descZh = new Dictionary<string, string> {
            ...
            { "Builder Enhancements\tGrid Override", "自由放置：方块不再吸附 1 单位网格，改为吸附 0.01 单位，可自由微调摆放位置。\n只改放置坐标取整精度，不改变玩法/物理；仅建造阶段生效。\n开启后无需按键即可自由摆放，关闭恢复标准网格吸附。" },
            { "Builder Enhancements\tGrid Toggle Key", "游戏内切换自由放置覆盖（默认 F2）" },
            ...
            { "Player Tracker\tTeleport Threshold", "瞬移清除距离（格数）：角色一帧内移动超过该距离（被瞬移/传送/派对盒选道具）时，删除已绘制的轨迹重新开始，避免拖出贯穿全图的长线。最小 1。" },
            ...
            { "实验\tQuick Switch", "快速切换（仅自由模式）：长按 B 到设定等待秒数切换 行动↔建造 模式（游戏默认 0.5 秒）。\n默认关闭；对局中开关立即生效。" },
            { "实验\tQuick Switch Hold", "快速切换的长按等待秒数（0-2，0=立即）。\n自由模式长按 B 达到该秒数切换行动/建造。" },
            { "实验\tQuick Retry", "死后自动重试（仅挑战模式）：死亡后自动触发重试（重开关卡），无需长按。\n原理：修改重生延迟在挑战模式的最短时间自动重试。\n默认关闭；对局中开关立即生效。" },
            { "实验\tQuick Retry Fast", "快速重试（仅挑战模式）：死亡后长按 B 到设定等待秒数自动重试。\n选中后由下方滑块条修改长按 B 的等待时间（游戏默认 0.5 秒）。\n默认关闭；对局中开关立即生效。" },
            { "实验\tQuick Retry Hold", "快速重试的长按等待秒数（0-2，0=立即）。\n挑战模式死亡后长按 B 达到该秒数自动重试。" },
            ...
            { "实验\tGrid Always On", "地图网格：建造（放置方块）阶段任何模式都显示网格；本开关让自由模式对局在行动阶段也保持网格（游戏默认行动阶段不显示）。\n其它模式行动阶段不会因本开关点亮网格。\n随开随关：在自由对局里开启立即淡入，关闭立即淡出。\n（开关在地图栏目里）" },
            ...
            { "Respawn\tDelay", "死亡后多少秒重生（最小 0.1 秒）：延迟后原地复活回出生点（仅自由模式）。\n挑战模式的重试等待请在「快速调整」页的「快速重试」分区设置。" },
            ...
            { "视野\t自由相机", "自由相机：开启后滚轮缩放视野（FOV 1-60）；关闭后完全恢复游戏默认相机。\n任何模式/场景都可用，但挑战模式对局内自动禁用。" },
            { "视野\tFOV", "视野大小（1 - 60）：数值越小拉得越近，越大看得越广（自由相机开启时生效，滚轮同步）" },
            ...
            { "设置\tDisabled Plugins", "被禁用的外部插件（GUID 列表，分号分隔；不在列表内的外部插件默认启用）" },
        ...
        private static readonly Dictionary<string, string> _descEn = new Dictionary<string, string> {
            ...
            { "Builder Enhancements\tGrid Override", "Free placement: pieces no longer snap to the 1-unit grid - they snap to 0.01 units, so you can fine-tune placement.\nOnly changes placement rounding precision, not gameplay/physics; build phase only.\nNo key needed while on; turning it off restores the standard grid snap." },
            { "Builder Enhancements\tGrid Toggle Key", "Toggle free placement in-game (default F2)" },
            ...
            { "Player Tracker\tTeleport Threshold", "Teleport clear distance (units): if a character moves farther than this in one frame (teleport/warp/party-box pick), delete the drawn trail and restart, so no giant line spans the map. Min 1." },
            ...
            { "实验\tQuick Switch", "Quick switch (freeplay only): hold B for the set seconds to swap Action<->Build (vanilla 0.5s).\nOFF by default; takes effect immediately." },
            { "实验\tQuick Switch Hold", "Hold seconds for quick switch (0-2, 0=instant).\nIn Freeplay, hold B this long to swap Action/Build." },
            { "实验\tQuick Retry", "Auto retry on death (challenge only): auto-retries (restart the run) after death, no hold.\nUses the respawn-delay challenge logic.\nOFF by default; takes effect immediately." },
            { "实验\tQuick Retry Fast", "Quick retry (challenge only): after death, hold B for the set seconds to auto-retry.\nWhen on, the slider below sets the hold-B wait time (vanilla 0.5s).\nOFF by default; takes effect immediately." },
            { "实验\tQuick Retry Hold", "Hold seconds for quick retry (0-2, 0=instant).\nIn Challenge, hold B this long after death to auto-retry." },
            ...
            { "Respawn\tDelay", "Seconds before respawn after death (min 0.1): respawns at the start point after the delay (freeplay only).\nFor the Challenge retry hold-time, see the Quick Retry section on the Quick Adjust page." },
            ...
            { "视野\t自由相机", "Free camera: wheel zooms the view (FOV 1-60); off fully restores the game camera (works in any mode/scene, auto-disabled in Challenge matches)" },
            { "视野\tFOV", "FOV (1 - 60): lower zooms in, higher sees more (applies when free camera is on; wheel syncs)" },
            ...
            { "设置\tDisabled Plugins", "Disabled external plugins (GUID list, semicolon-separated; plugins not on the list are enabled by default)" },
