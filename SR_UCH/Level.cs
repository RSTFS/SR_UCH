...
//本文件反射的游戏私有成员（游戏更新改名时：日志告警 + 可在 cfg [Reflection] 段填新名恢复）
    ...
    public void Initialize(MainPlugin plugin) {
        SelfReg();
			...
			//同类写法见 Experiments.cs:223 / Freeplay.cs:58 / QuickAdjust.cs:178。
			//带 try/catch：单个补丁目标失效（游戏更新改方法名）只丢本类功能，不影响其它功能 ——
			//参考 BuilderEnhancements.cs:119（它是全项目唯一原本就分开注册 + 容错的地方）。
        try { Harmony.CreateAndPatchAll(typeof(LevelTools), (string)null); }
        catch (Exception e) { MainPlugin.ModLogger.LogError("LevelTools 补丁注册失败: " + e.Message); }
    }
		...
		//对象生命周期结束等）就没人来复位 → 「重载关卡」与「广播快照」会永远提示"正在重载"。
		...
		private static bool ReloadBlocked()
		{
			...
			if (ReloadLockStuck()) {
				SetReloadBusy(false);
				MainPlugin.ModLogger.LogWarning("[重载] 检测到残留的忙碌锁（超过 45 秒未复位），已自动放锁");
				return false;
			}
			Experiments.NotifyExp(SR.T("正在重载，请稍候", "Reload in progress"));
			return true;
		}
		...
		private static void ReloadLevelImpl(string feature)
		{
			...
			try
			{
				...
				//残留 true，之后「重载关卡」与「广播快照」（同一把锁）会永远提示"正在重载"，
...
// ==== 分区：Reload（重载关卡保分补分 / 折扣保留 / 重载宽限 / 加载后 GC）====
    ...
    private static void FillScoresIfPending() {
        ...
        try {
            ...
            if (capped) {
                MainPlugin.ModLogger.LogWarning("[补分] 备份分块 " + total + " 条超过上限 " + FillTotalCap
