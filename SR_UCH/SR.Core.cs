...
// 约定：静态字段集中在此；新功能实现 ITweak 即被自动发现，新分区需在 sr_uch.rsp 注册。
        ...
        public static void RegisterCultivation(object cultivation) {
            ...
            //EX 栏目的侧栏注册在 SR.Pages 的 SelfRegPages 里
        }
        ...
        //「无视模式限制」「冻结角色」是 EX 模块的开关（配置/按键/界面都在 EX）。
        //SR 侧只保留读取接点：EX 初始化时调 SetExToggles 注册取值委托；未装 EX = 都关闭（原版行为）。
        ...
        public void Initialize(MainPlugin plugin) {
            ...
            _langEntry = plugin.Config.Bind("Settings", "Language", "English", "界面语言：中文 / English（EX 附加页始终中文）");
            ...
            //EX 页的两个开关（无视模式限制 / 冻结角色）已随功能下放到 EX 模块（Bind 也在那边）
...
// 反射探测型 catch（ExRef 找可选成员，成员不存在是常态）不接入——那是预期分支，不是错误。
