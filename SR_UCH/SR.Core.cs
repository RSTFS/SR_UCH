...
// 约定：静态字段集中在此；新功能实现 ITweak 即被自动发现，新分区需在 sr_uch.rsp 注册。
        ...
        // ---- 反射辅助（下放版）：成员名可 cfg 覆盖 + 失效告警 ----
        // 本模组反射游戏的私有成员（见各功能文件）。游戏一更新，任一成员改名都会让
        // AccessTools 静默返回 null —— 表现为"功能装了没反应"，过去没有任何提示。
        // 这里只提供三个取成员的入口（不做集中注册表）：
        //   · 成员名优先取 cfg 的 [Reflection] 段（游戏更新后用户填新名字即可恢复，不必等发新版）；
        //   · 取不到就打一条 Warning，说明是哪个成员、它支撑什么功能（去重，不刷屏）。
        // 各功能文件在 SelfReg 里用 SR.RefOverride 声明"成员 → 支撑什么功能"，
        // 调用处用 SR.RefField / SR.RefMethod 取成员（与 LocKey/LocDesc 同一套自包含写法）。
        private static readonly Dictionary<string, string> _refFeature = new Dictionary<string, string>();
        private static readonly Dictionary<string, ConfigEntry<string>> _refOverride = new Dictionary<string, ConfigEntry<string>>();
        ...
        public static void RefOverride(string cfgKey, string feature) {
            if (!string.IsNullOrEmpty(cfgKey)) _refFeature[cfgKey] = feature;
        }

        //按 cfg 覆盖值取实际成员名（无覆盖则用原名）；惰性建配置项（此时 _internalConfig 已就绪）
        private static string RefName(Type owner, string name) {
            string key = owner.Name + "." + name;
            ConfigEntry<string> e;
            if (!_refOverride.TryGetValue(key, out e)) {
                if (_internalConfig == null) return name;
                try {
                    string feat;
                    _refFeature.TryGetValue(key, out feat);
                    e = _internalConfig.Bind("Reflection", key, "",
                        "游戏更新后该成员若改名，在这里填新名字即可恢复（留空 = 用内置默认名）。\n支撑功能：" + (feat ?? "未登记"));
                    _refOverride[key] = e;
                } catch { return name; }
            }
            try {
        ...
        private static void RefWarn(Type owner, string name, string feature) {
            ...
            try {
                MainPlugin.ModLogger.LogWarning("[SR.Ref] 反射失效: " + key + " → " + (feat ?? "未登记")
                    + " 已禁用（游戏可能已更新；可在 cfg 的 [Reflection] 段把 " + key + " 填成新成员名恢复）");
            } catch { }
        }

        public static FieldInfo RefField(Type owner, string name, string feature) {
            try {
                FieldInfo f = AccessTools.Field(owner, RefName(owner, name));
                if (f != null) return f;
            } catch { }
            RefWarn(owner, name, feature);
            return null;
        }

        public static MethodInfo RefMethod(Type owner, string name, string feature) {
            try {
                MethodInfo m = AccessTools.Method(owner, RefName(owner, name));
                if (m != null) return m;
            } catch { }
            RefWarn(owner, name, feature);
            return null;
        ...
        public static MethodInfo RefMethod(Type owner, string name, Type[] parameters, string feature) {
            try {
                MethodInfo m = AccessTools.Method(owner, RefName(owner, name), parameters);
                if (m != null) return m;
            } catch { }
            RefWarn(owner, name, feature);
            return null;
        ...
        private static Type RefType(string simpleName) {
            try {
                ...
                foreach (Assembly a in AppDomain.CurrentDomain.GetAssemblies()) {
                    try { t = a.GetType(simpleName, false); if (t != null) return t; } catch { }
                }
            } catch { }
            return null;
        }
        ...
        public static void RefAudit() {
            ...
            try {
                ...
                MainPlugin.ModLogger.LogInfo("[SR.Ref] 反射成员 " + _refFeature.Count + " 个：可用 " + ok + "，失效 " + failed.Count);
                for (int i = 0; i < failed.Count; i++) {
                    MainPlugin.ModLogger.LogWarning("[SR.Ref]   ✗ " + failed[i]
                        + " → 该功能已禁用；游戏若已更新，可在 cfg [Reflection] 段填新成员名恢复");
                }
        ...
        public void Initialize(MainPlugin plugin) {
            ...
            //反射探针：绑定 [Reflection] 段的「成员名覆盖」项 + 启动自检。
            //游戏更新导致私有成员改名时，日志会列出失效项及它支撑的功能，
            //用户可在 cfg 里填新成员名恢复，不必等发新版。必须在迁移/重读之后调用。
            //[Reflection] 是纯内部段（成员名覆盖用），不占侧栏栏目
