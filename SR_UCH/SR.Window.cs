...
// ==== 分区：Window（主窗口框架：Tick 主循环 / 标题栏 / DrawGUI / 窗口拖动缩放）====
        ...
        private static void Tick() {
            ...
            //「冻结角色」（EX 页控制，默认关）：只冻结自己的角色（见 FreezeLocalCharacter），不暂停全局时间 → 其他角色/动画照常移动，保留原版观感。
            if (_pauseApplied) {
        ...
        private static void DrawGUI() {
            ...
            //EX 按钮右键绑键由按钮绘制当帧自判（跨帧登记矩形会被状态栏增减行挤位移 → 绑错键）；
            ...
            if (_mode == Mode.Internal) {
                if (_selectedInternalSection == "Home") {
                    ...
                    //兜底：纯通用条目列表（没有自绘页的栏目）
                    bool any = false;
                    //附加模块没注册页面（通常是 EX 版本过旧）：提示更新，避免“装了却没反应”
                    if (_selectedInternalSection == "EX" && ExRef.Loaded && !ExRef.VersionOk) {
