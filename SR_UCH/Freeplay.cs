...
public class Freeplay : ITweak {
        ...
        public void Initialize(MainPlugin plugin) {
            ...
            try { Harmony.CreateAndPatchAll(typeof(Freeplay), (string)null); } catch (Exception e) { MainPlugin.ModLogger.LogWarning("自由模式 补丁注册失败: " + e.Message); } //独立类不会随 SR 自动注册
        }
...
// ==== 分区：Freeplay（独立功能类：地图视图 + 地图网格 + 地图状态与配置）====
        ...
        internal static void ApplyMapViewOnCamera(Camera cam) {
            ...
            //注：本方法开头已强制 cam.orthographic = false，故不再有正交分支（旧分支为死代码，已删）。
            //地图用自由相机（FOV 取自视野页配置，默认 10）：滚轮直接调 FovAdjust，视野页"当前 FOV"自动跟随。
            float fov = Mathf.Clamp(FovAdjust.FovValue, 1f, 32f); //上限与视野页一致（FovAdjust.MaxFov = 32）
            cam.fieldOfView = fov;
        ...
        //玩家标记用的角色列表帧缓存（DrawMapWindow 每帧调用，避免全场景扫描）
        private static Character[] _mapCharCache;
        private static int _mapCharCacheFrame = -1;
        ...
        internal static void DrawMapWindow() {
            ...
            //玩家标记每帧缓存：DrawMapWindow 由 OnGUI 每帧调用（且每帧至少两次：Layout + Repaint），
            //每次都 FindObjectsOfType<Character>() 会全场景扫描。角色集合一帧内不会变，
            //按 Time.frameCount 复用同一份结果（与 EX 侧 PlayerTable 同一写法）。
            if (_mapCharCache == null || _mapCharCacheFrame != Time.frameCount) {
                _mapCharCacheFrame = Time.frameCount;
                _mapCharCache = UnityEngine.Object.FindObjectsOfType<Character>();
            }
            foreach (Character c in _mapCharCache) { //players
                if (c == null) continue;
        ...
        public static void Render() {
            ...
            if (SR.Ctl.HotkeyButton("map.master",
                Enabled ? SR.T("地图总开关：开", "Map Master: ON") : SR.T("地图总开关：关", "Map Master: OFF"),
                SR.T("地图总开关：关闭后 M 键无法打开地图", "Map master switch: when off the M key can't open the map"),
                () => SR.Ctl.SetValue(SR.Ctl.MapEnabledEntry, !Enabled), SR.Ctl.Sc(170), SR.Ctl.Sc(30))) {
            ...
            if (tm != null) SR.Ctl.RenderControl(tm);
            GUILayout.FlexibleSpace();
            GUILayout.EndHorizontal();
