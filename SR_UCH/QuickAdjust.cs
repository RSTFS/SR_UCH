...
//本文件反射的游戏私有成员（游戏更新改名时：日志告警 + 可在 cfg [Reflection] 段填新名恢复）
    ...
    public void Initialize(MainPlugin plugin) {
        SelfReg();
			_scoreDiscount = ((BaseUnityPlugin)plugin).Config.Bind<int>("Quick Adjust", "Score Discount", 20, "评分折扣 %：把自己的得分平衡板 handicap 设为 100-折扣 %（范围 0-90：滑块模式整十，勾选「更多折扣数值」后可填任意整数如 85 → handicap 15%；上限 90 = handicap 10；0 = 关闭；只影响自己）。\n默认 20（handicap 80%）；平衡板上自己那一行显示为对应百分比；可随时点“恢复原值”还原。");
        ...
        //带 try/catch：单个补丁目标失效（游戏更新改方法名/签名）只丢本类功能，不影响其它功能。
			...
			catch (Exception e) { MainPlugin.ModLogger.LogError("快速调整 补丁注册失败: " + e.Message); }
    }
...
public class TreehouseSuicide : ITweak {
    ...
    public void Initialize(MainPlugin plugin) {
        ...
        catch (Exception e) { MainPlugin.ModLogger.LogError("快速自杀 补丁注册失败: " + e.Message); }
    }
