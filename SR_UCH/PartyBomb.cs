...
namespace SR_UCH.Tweaks {
    //派对盒子炸弹：房主开启开关后，对局内**所有在线成员**都发送过「炸弹！」
    //快捷短语（EmoteMeanings.EMOTE_Bomb，按枚举码识别，与多语言/文本无关）时，房主
    //往当前派对盒子塞入一个可选取的炸弹。炸弹型号可配置（PartyBox.BombPrefab 里的
    //第几个：0/1/2... 对应不同大小的炸弹）。
    //⚠ 实验功能：涉及动态改 PartyBox 私有 pieces + 网络同步，需游戏内验证。
    public class PartyBomb : ITweak {
        ...
        public void Initialize(MainPlugin plugin) {
            _enabled = plugin.Config.Bind("EX", "Party Bomb", false,
                "派对盒子炸弹：房主开启后，对局内所有成员都发过「炸弹！」快捷短语时才往派对盒子塞炸弹。仅房主生效。");
            _bombSize = plugin.Config.Bind("EX", "Party Bomb Type", 0,
        ...
        //快捷短语都走 ChatDisplay.DisplayNewMessage，EmoteType 保留原枚举码。
        //房主端收集每位成员的 EMOTE_Bomb；全员都发过才生成一次炸弹。
