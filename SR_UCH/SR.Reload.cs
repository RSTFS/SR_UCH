...
// ==== 分区：Reload（重载关卡保分补分 / 折扣保留 / 重载宽限 / 加载后 GC）====
        ...
        static void OnPrepareReloadMessage(GameEvent.GameEvent e) {
            try {
                //收到 PrepareToReloadScene。SR 只对“自己发起的重载”补分（Experiments.ReloadLevelImpl
                //里已直接 MarkPreserveScores 置标）；这里**不再**因为收到消息就自动置补分标志——
                //否则外部工具（如独立 Level Tool）发起的同一次重载会被 SR 再补一份 → 分数翻倍。
