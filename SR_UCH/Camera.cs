...
namespace SR_UCH.Tweaks {
    ...
    public class FovAdjust : ITweak {
        ...
        public static void SetFov(float v) {
            ...
            _fovEntry.Value = Mathf.Clamp(v, MinFov, MaxFov); //落盘由 SR.FlushConfig 节流（滚轮每格不写盘）
        }
        ...
        public static void TickInput() {
            ...
            if (Mathf.Abs(wheel) >= 0.0001f) {
                if (cam.orthographic) {
                    ...
                    //去掉后一格约 0.7 档，与 1-32 滑块粒度匹配（地图滚轮 wheel*3f 亦同数量级）。
                    float nv = cam.fieldOfView - wheel * ZoomSensitivity;
